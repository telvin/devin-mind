...
class MockDevinClient {
    ...
    async getSession(sessionId) {
        ...
        // If we should complete and delay is reached, return completed session
        if (this.shouldComplete && this.pollCount > this.completionDelay) {
            return {
                message_count: 2,
                raw_response: {
                    messages: [
...
describe('HandoffManager', () => {
    ...
    describe('Polling Settings', () => {
        ...
        it('should use different intervals for first poll vs subsequent polls', async () => {
            ...
            manager.setFirstPollingInterval(100); // 100ms for first poll
            manager.setPollingInterval(50); // 50ms for subsequent polls
            ...
            // Should have taken at least the time for intervals: 100ms (first) + 50ms (second) = 150ms
            expect(endTime - startTime).toBeGreaterThan(140); // Allow some tolerance
        ...
        it('should complete immediately when session is ready on first poll', async () => {
            ...
            manager.setPollingInterval(100);
            manager.setFirstPollingInterval(200);
        ...
        it('should handle maxPolls of 1 correctly', async () => {
            ...
            // Set maxPolls to 1
            manager.setPollingInterval(10);
            manager.setFirstPollingInterval(10);
