export class HandoffManager {
  ...
  - Technical details or artifacts created`;
    ...
    async pollSessionUntilSessionDone(sessionId, maxPolls = 9999) {
        let pollCount = 0;
        while (pollCount < maxPolls) {
            try {
                const session = await this.devinClient.getSession(sessionId);
                ...
                if (pollCount < maxPolls) {
                    ...
                    console.log(`Polling ${sessionId} (Done): attempt ${pollCount}/${maxPolls}, interval: ${this.formatDuration(currentInterval)}, elapsed: ${elapsed}`);
